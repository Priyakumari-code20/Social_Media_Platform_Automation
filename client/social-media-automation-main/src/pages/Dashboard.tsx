import { Share2Icon, CheckCircleIcon, ClockIcon, TrendingUpIcon, ActivityIcon, SendIcon } from "lucide-react"
...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        const [postsRes, accountsRes, activityRes] = await Promise.all([api.get("/api/posts"),
              ...
              <div key={activity._id} className="flex items-start gap-4 px-6 py-4 hover:bg-slate-50/50 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2 mb-1">
                    ...
                    <span className="text-xs text-slate-400 shrink-0">{new Date(activity.createAt).toLocaleString()}</span>
                  </div>
