...
const Accounts = () => {
  ...
  const fetchAccounts = async(isSync = false, platform?: string | null, successMsg?: string) => {
    try {
      if(isSync){
        ...
        "Social Media";
        toast.loading(`Syncing ${label} acccount...`,{id:"sync"}) ; 
  ...
  return (
    <div className="space-y-8 max-w-4xl">
     ...
     {showPlatformPicker && <PlatformPickerModal connectedIds={connectedIds} 
     connecting={connecting} onClose={() => setShowPlatformPicker(false)} onConnect={handleConnect} />}
