...
const Sidebar = ({isOpen, setIsOpen} : {isOpen:boolean, setIsOpen: (val: boolean) => void}) => {
  ...
  return (
    ...
    <div className='px-6 py-2'>
      <span className='text-xs text-slate-500 uppercase tracking-order'> Menu</span>
    </div>
      ...
      {navItems.map((item) => {
        ...
        return (
          <NavLink key={item.name}
           ...
           className={`flex items-center gap-2 px-3 py-2.5 rounded text-sm transition-all duration-150 border 
    ...
    <div className = "p-4 border-t border-slate-100">
      ...
      <button onClick={logout} className="mt-1 flex items-center gap-2 px-3 py-2 w-full rounded:
       text-sm text-slate-500 hover:bg-red-50 hover:text-red-500 transition-all duration-150">
