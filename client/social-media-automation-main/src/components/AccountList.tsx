...
const AccountList = ({accounts, onDisconnect}: AccountListProps) => {
    ...
    if(accounts.length === 0){
        return (
            <div className="bg-white rounded-2xl  border border-dashed border-slate-200 flex flex-col items-center justify-center py-20 px-6">
                ...
                <p className="text-sm text-slate-400  mt-1 max-w-xs text-center">Connect your social media accounts to start scheduling and automating your content.</p>
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {accounts.map((account,index)=>{
           ...
           return(
            <div key ={index} className="group bg-white border border-slate-200 rounded-2xl p-5 flex items-center gap-4 hover:border-slate-300 trnsition-all">
